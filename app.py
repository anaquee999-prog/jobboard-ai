...
from doe_sources import DOE_NEWS_SOURCES, PRIMARY_DOE_SOURCE_URL
...
def _safe_agency_name(company_name="", location="", source_url=""):
    ...
    bad = {"", "ระบบอัตโนมัติ", "auto job engine", "government job news"}

    if company_name.lower() not in bad:
        return company_name

    for source in DOE_NEWS_SOURCES[1:]:
        if source["key"] in source_url or source["province"] in location:
            return source["employer"]
    return DOE_NEWS_SOURCES[0]["employer"]
